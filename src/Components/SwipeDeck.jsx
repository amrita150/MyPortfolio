...
const SwipeDeck = () => {
  ...
  const swipePrev = () => {
    if (currentIndex > 0) {
      ...
      navigate(`/${sections[prevIndex].id}`);
    }
  };
            ...
            (sec, idx) =>
              idx === currentIndex && (
                <motion.div
                  ...
                  dragElastic={0.2}
                  onDragEnd={(e, info) => {
                    if (info.offset.x < -100) swipeNext();
                    else if (info.offset.x > 100) swipePrev();
                  }}
                  initial={{ x: 300, opacity: 0 }}
                  animate={{ x: 0, opacity: 1 }}
                  exit={{ x: -300, opacity: 0 }}
                  transition={{ duration: 0.3 }}
                 className="absolute inset-0 rounded-2xl overflow-y-auto hide-scrollbar transition-all duration-300 shadow-2xl"
                >
                  {sec.component}
                </motion.div>
              )
